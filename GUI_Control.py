...
def put_label(frame, put_row, count):
    if put_row[4] == "0":
        ...
        Label(frame, text='Departamento: ' + put_row[1]).grid(row=count, column=1, sticky=W, padx=(0, 5))
        Label(frame, text='Problema: ' + put_row[2]).grid(row=count, column=2, sticky=W, padx=(0, 7))
        ...
        Button(frame, text='OK', state=DISABLED).grid(row=count, column=7, padx=(0, 7), pady=(3, 3))
                ...
                if time.ctime(os.path.getmtime(tools.path_vendas)) != tempo:
                    ...
                    for row in reader:
                        if row not in self.data:
                            ...
                            putlabel(self.frame, row, self.count)
                            self.count = + 1
...
root = Tk()
root.title('MaxiDesk')
root.geometry('1200x600+' + str(int(GetSystemMetrics(0) / 2 - 600)) + '+' + str(int(GetSystemMetrics(1) / 2 - 300)))
